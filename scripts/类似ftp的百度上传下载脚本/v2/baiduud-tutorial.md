...
本文档总结 `baiduud.cmd` 与 `baiduud.sh` 的完整需求、参数、使用案例、运行流程和排错方法，并附带两份主脚本及 Windows 表格辅助脚本的完整源码，便于以后直接查阅。

- 更新日期：2026-07-17
- Windows 脚本：[baiduud.cmd](./baiduud.cmd)
- Windows 表格辅助脚本：[baiduud-table.ps1](./baiduud-table.ps1)
- Linux 脚本：[baiduud.sh](./baiduud.sh)
- Windows SHA-256：`681C170C7C9E9BE352EA4D3DDDF56803158F527321C1710690A835948D08F8B3`
- Windows 表格辅助脚本 SHA-256：`94FBE79C2CD2E53F8E684E519CE7A109A6C876638790B590762D3B062E82A7E2`
- Linux SHA-256：`CF51BD8644885E37B8D1303003D8F4E482BEAF58CF86F1332CC8957438F5F964`
...
19. 将下载和 `list` 的 Markdown 风格列表改为动态列宽的终端边框表格，英文和中文内容都能对齐。
20. 修复 Windows 网盘列表中文名称乱码：将 `baiducmd ls -l` 的 UTF-8 重定向结果转换为 GBK 后再交给 CMD 解析。
...
`list` 使用默认 `BAIDU_UPLOAD` 或 `-bu` 指定目录，输出与 `baiduud d` 相同的序号、名称、类型、大小、修改日期五列终端表格。表格使用 `+-----+` 边框并按当前内容动态计算列宽，不再输出 Markdown 分隔语法。它不会显示下载选择提示，也不会上传、下载或删除数据。
...
输出格式与 Windows `list` 和下载列表一致，使用动态列宽的终端边框表格，不进入下载选择流程。
...
3. 创建并验证网盘下载目录。
4. 网盘目录为空时正常结束。
5. Windows 先把 `ls -l` 的 UTF-8 重定向结果转换为 GBK；随后解析项目、移除名称右侧用于客户端对齐的空格，并显示从 1 开始、动态列宽的五列终端边框表格。Linux 直接按 UTF-8 解析。
...
2. 自动创建缺失的网盘目录，并通过 `ls -l` 验证目录可访问；Windows 在解析前执行 UTF-8 到 GBK 转换。
3. 复用下载流程的项目解析和终端表格打印函数，输出从 1 开始的名称、类型、大小和修改日期，中文名称会原样保留。
4. 空目录记录中文提示并正常结束。
...
`baiducmd ls -l` 被重定向到临时文件时输出 UTF-8，而 CMD 的 `findstr` 和 `for /f` 按代码页 936 读取。当前版本会先严格按 UTF-8 解码并转写为 GBK，再解析中文名称；转换失败时只显示中文错误并停止，避免把乱码继续传入下载路径。

`baiduud-table.ps1` 必须使用 UTF-8 BOM 编码，并与 `baiduud.cmd` 放在同一目录。它只负责根据 CMD 已解析的项目生成动态列宽终端表格；缺失或执行失败时，主脚本会显示中文错误并返回退出码 `1`。
...
本文档附录中的源码应与实际脚本保持一致。上方三项 SHA-256 用于判断脚本是否已经变化。
